...
int primsAlgorithm(int graph[N][N], int size)
{
	...
	for(path=0; path<size; path++){
		edge = valueMST(pathValue, visited, size); // "valueMST" fonksiyonu cagirilir. "edge" degiskenine fonksiyonun dondurdugu "minIndex" degeri atanir.
		visited[edge] = true; // "edge" degerine atanan dugum numarasinin ziyaret bilgisine '1' degeri atanir.
		
		for(j=0; j<size; j++){
			counter++; // Zaman karmasikligi icin ic ice dongulere giris sayisi hesaplanir.
			
			if(graph[edge][j] != 0 && visited[j] == false && graph[edge][j] < pathValue[j]){
				pathValue[j] = graph[edge][j];
				parent[j] = edge;
			}
		}
	}
